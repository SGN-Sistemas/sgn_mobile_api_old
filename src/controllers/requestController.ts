...
export default class DailyMovimentController {
  public async list (request: Request, response: Response): Promise<Response> {
    ...
    const execute = await listPedidoService.execute(userId, '');
  ...
  public async listNumber (request: Request, response: Response): Promise<Response> {
    ...
    const execute = await listPedidoNumberService.execute(userId, queryString);
  ...
  public async listForn(request: Request, response: Response): Promise<Response> {
    ...
    const execute = await listPedidoFornService.execute(userId, queryString);
  ...
  public async listFunc(request: Request, response: Response): Promise<Response> {
    ...
    const execute = await listPedidoFuncService.execute(userId, queryString);
  ...
  public async approvalRequest (request: Request, response: Response): Promise<Response> {
    ...
    for (let i = 0; i < sqlExec.length; i++) {
      const valid = await validCereFornPedi(userId, sqlExec[i].CERE_COD, valTotal, fornCod, pediCod);
      if (valid.error === true) {
        return response.status(valid.status).json(valid);
      }
    }

    const approvalRequestExec = await approvalRequestService.execute(userId, USUA_SENHA_APP, posUsuaCod, pediCod, pediNumero);
  ...
  public async approvalLargeScale(request: Request, response: Response) {
    ...
    for await (const item of arrayPedido) {
      ...
      for (let i = 0; i < sqlExec.length; i++) {
        ...
        const valid = await validCereFornPedi(userId, sqlExec[i].CERE_COD, item[2], item[3], item[1]);
        ...
        if (i === sqlExec.length || valid.error === false) {
          const approvalRequestService = new ApprovalRequestService();
          const approvalRequestExec = await approvalRequestService.execute(userId, USUA_SENHA_APP, item[0], item[1], item[4]);
