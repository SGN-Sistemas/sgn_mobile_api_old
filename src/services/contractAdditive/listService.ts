...
export class ListServiceContractAdditive {
  public async execute (cod: string, database: string): Promise<IReturnResponse> {
    try {
      const sql = selectAditivoContrato1(cod, '', database)
      const sql2 = selectAditivoContrato2(cod, '', database)
      const sql3 = selectAditivoContrato3(cod, '', database)
      const sql4 = selectAditivoContrato4(cod, '', database)
